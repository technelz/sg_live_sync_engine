...
def build_plan(
    ...
    # match_security_groups now returns 4 values
...
def detect_security_group_drifts(
    ...
    for m in matches:
        ...
        # desired state computed from source (with dependency rewrite)
        ...
        missing_ingress, extra_ingress = split_rule_diff(current_ingress, desired_ingress)
        missing_egress, extra_egress = split_rule_diff(current_egress, desired_egress)
...
def create_sg_shell(
    ...
    try:
        resp = context.ec2.create_security_group(
            GroupName=name,
...
def apply_rules_from_plan(
    ...
    for match in plan.matches:
        ...
        try:
            ...
            # POLICY: do NOT remove extra rules unless explicitly allowed
...
def validate_execution_safety(args: Args, sg_plan: SgPlan) -> None:
    ...
    if args.yes and sg_plan.unresolved_dependencies and not args.allow_unresolved_dependencies:
        raise ValueError(
            "Unresolved SG rule dependencies exist. Review report or use "
            "--allow-unresolved-dependencies only if acceptable."
        )
...
def main() -> int:
    ...
    try:
        ...
        # PLAN
        sg_plan = build_plan(source_groups, target_groups, args.allow_legacy)
